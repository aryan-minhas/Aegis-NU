...
const GRADE_POINTS = {
  ...
  F: 0.0,
  I: null,
  W: null, // Incomplete/Withdrawn don't affect GPA
};
...
function initGPAModule() {
  ...
  liveStatsContainer.className =
    "aegis-live-gpa flex gap-4 p-3 mt-2 rounded border border-blue-200 bg-blue-50";
  ...
  rows.forEach((row, index) => {
    ...
    select.className =
      "border border-slate-300 rounded p-1 text-sm bg-white cursor-pointer hover:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-200 transition-all";
  ...
  function recalculateGPA() {
    ...
    const newEstimatedCgpa =
      (estimatedBasePoints + ptsDifference) / totalEarnedCrHrs;
