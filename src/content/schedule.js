...
function initScheduleBuilder() {
  ...
  container.className = 'my-6 mx-4 p-6 bg-white rounded-xl shadow-lg border border-gray-200 font-sans';
  container.innerHTML = `
  ...
  const targetNode = document.querySelector('.page-content') || document.querySelector('#page-wrapper') || document.body;
    ...
    if (headers.some(h => h.includes('course') || h.includes('title'))) {
      ...
      rows.forEach((row, rowIndex) => {
        ...
        if (cells.length > 2 && !row.querySelector('.aegis-add-btn')) {
          ...
          addBtn.className = 'aegis-add-btn px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded shadow transition-colors text-sm';
          addBtn.innerHTML = '➕ Add to Sandbox';
          
          addBtn.addEventListener('click', (e) => {
            e.preventDefault();
            // Assign a pseudo-random day and time for demonstration if not parsed
            // In a real scenario, this would parse the specific section times
            const randomDay = DAYS[Math.floor(Math.random() * DAYS.length)];
            const randomSlot = Math.floor(Math.random() * (TIME_SLOTS.length - 2)); // 2-3 hour duration
...
function renderTimetable() {
  // Clear all cells
  document.querySelectorAll('#aegis-timetable-body td[data-day]').forEach(td => {
    td.innerHTML = '';
  ...
  // Render blocks
  sandboxSchedule.forEach(course => {
    const firstCell = document.querySelector(`td[data-day="${course.day}"][data-slot="${course.startSlot}"]`);
    if (firstCell) {
      ...
      // Style as absolute to span across cells if needed, or simply render inside the first cell with absolute positioning
      // For simplicity in a basic table, we render an absolutely positioned div relative to the cell
      ...
      // Calculate width based on duration (assuming each td is roughly 120px wide or taking % of table)
      // Since it's absolutely positioned within a relatively positioned TD, we can set width like:
