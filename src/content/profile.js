...
if (window.location.href.includes('StudentProfile') || window.location.href.includes('Home')) {
    setTimeout(initProfileModule, 1500);
}

function initProfileModule() {
    // Look for the Academic Calendar section
    const headings = Array.from(document.querySelectorAll('h1, h2, h3, h4, h5, .caption-subject'));
    ...
    const container = calendarHeader.closest('.portlet, .card, .dashboard-stat') || calendarHeader.parentNode;

    // Inject download button
    const btn = document.createElement('button');
    btn.innerText = '📅 Download .ics Calendar';
    btn.className = 'aegis-ics-btn bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold py-2 px-4 rounded-md shadow-sm transition-colors mt-2 mb-4';
    
    // Inline fallback
    btn.style.cssText = 'background-color: #2563eb; color: #fff; font-size: 0.875rem; font-weight: 600; padding: 8px 16px; border: none; border-radius: 6px; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; margin-top: 8px; margin-bottom: 16px; font-family: system-ui; box-shadow: 0 1px 2px 0 rgba(0,0,0,0.05);';
...
function generateAndDownloadICS(container) {
    ...
    items.forEach(item => {
        ...
        // FlexStudent formats typically look like: "Classes: 15-Jun-2026 to 07-Aug-2026" or "Registration: 10-Jun-2026"
