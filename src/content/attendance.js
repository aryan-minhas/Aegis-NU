...
async function initAttendanceModule() {
  try {
    ...
    if (courseData.length === 0) {
      console.warn(
        "Aegis-NU: No course data found in local storage. Cannot calculate Safe to Skip.",
      );
...
function parseAttendanceDOM(courseData) {
  ...
  tables.forEach((table) => {
    ...
    // Fallback: Since background fetching is disabled for session safety,
    // we assume a standard 3 Credit Hour course (allows 6 skips).
    // If it's matched in storage (from a manual visit), use the real CH.
    let assumedCreditHours = matchedCourse ? matchedCourse.creditHours : 3;

    const allowedAbsents =
      assumedCreditHours === 1 ? 3 : Math.floor(assumedCreditHours * 2);
...
function injectBadge(
  ...
  if (
    targetTable.previousElementSibling &&
    targetTable.previousElementSibling.classList.contains("aegis-badge")
  ) {
    return;
  }
  ...
  badgeContainer.className =
    "aegis-badge flex flex-wrap items-center justify-between bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4 shadow-sm font-sans";
